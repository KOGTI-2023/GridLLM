...
export const inferenceRoutes = (
  ...
  router.post('/', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!availableModels.includes(validatedData.model)) {
      throw createError(`Model '${validatedData.model}' is not available on any worker`, 400);
    }

    // Check if there are available workers for this model
    const availableWorkers = workerRegistry.getAvailableWorkersByModel(validatedData.model);
    if (availableWorkers.length === 0) {
      throw createError(`No available workers for model '${validatedData.model}'`, 503);
    }
  ...
  router.get('/models', asyncHandler(async (req: Request, res: Response) => {
    ...
    for (const worker of allWorkers) {
      for (const model of worker.capabilities.availableModels) {
        ...
        if (existing) {
          existing.workers++;
          if (worker.status === 'online' && worker.currentJobs < worker.capabilities.maxConcurrentTasks) {
          ...
          models.set(model.name, {
            name: model.name,
            workers: 1,
            availableWorkers: (worker.status === 'online' && worker.currentJobs < worker.capabilities.maxConcurrentTasks) ? 1 : 0,
    ...
    const modelList = Array.from(models.values()).sort((a, b) => a.name.localeCompare(b.name));
