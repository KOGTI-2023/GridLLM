...
export const ollamaRoutes = (
  ...
  const generateRequestSchema = Joi.object({
    ...
    keep_alive: Joi.alternatives().try(
      Joi.string(),
  ...
  const validateModelAvailability = (model: string) => {
    const availableModels = workerRegistry.getAllAvailableModels();
    if (!availableModels.includes(model)) {
      throw createError(`Model '${model}' is not available`, 404);
    }

    const availableWorkers = workerRegistry.getAvailableWorkersByModel(model);
    if (availableWorkers.length === 0) {
      throw createError(`No available workers for model '${model}'`, 503);
    }
  };
  ...
  router.post('/api/generate', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = generateRequestSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }

    validateModelAvailability(validatedData.model);
    ...
    try {
      if (validatedData.stream) {
        ...
        const ollamaResponse = convertToOllamaResponse(result, validatedData.model);
  ...
  router.post('/api/chat', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = chatRequestSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
    ...
    const prompt = validatedData.messages
  ...
  router.post('/api/create', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = createModelSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
  ...
  router.get('/api/tags', asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post('/api/show', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = showModelSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
    ...
    for (const worker of allWorkers) {
      const model = worker.capabilities.availableModels.find(m => m.name === validatedData.model);
      if (model) {
        modelInfo = model;
        break;
      }
  ...
  router.post('/api/copy', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = copyModelSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }

    validateModelAvailability(validatedData.source);
  ...
  router.delete('/api/delete', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = deleteModelSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
  ...
  router.post('/api/pull', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = pullModelSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
    ...
    if (validatedData.stream) {
      ...
      const steps = [
        ...
        { status: 'downloading', digest: 'sha256:example', total: 1000000, completed: 250000 },
        { status: 'downloading', digest: 'sha256:example', total: 1000000, completed: 500000 },
        { status: 'downloading', digest: 'sha256:example', total: 1000000, completed: 1000000 },
  ...
  router.post('/api/push', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = pushModelSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
    ...
    if (validatedData.stream) {
      ...
      const steps = [
        ...
        { status: 'starting upload', digest: 'sha256:example', total: 1000000 },
  ...
  router.post('/api/embed', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = embedRequestSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
    ...
    const inputs = Array.isArray(validatedData.input) ? validatedData.input : [validatedData.input];
    ...
    res.json({
      ...
      prompt_eval_count: inputs.reduce((sum: number, input: string) => sum + input.split(' ').length, 0),
    });
  }));

  // API/PS - List running models
  router.get('/api/ps', asyncHandler(async (req: Request, res: Response) => {
    const allWorkers = workerRegistry.getAllWorkers();
    const runningModels = [];
    ...
    for (const worker of allWorkers) {
      if (worker.status === 'online' && worker.currentJobs > 0) {
        ...
        for (const model of worker.capabilities.availableModels.slice(0, 1)) { // Simulate one loaded model per worker
          runningModels.push({
            name: model.name,
            model: model.name,
            size: model.size,
  ...
  router.post('/api/embeddings', asyncHandler(async (req: Request, res: Response) => {
    const { error, value: validatedData } = embeddingsRequestSchema.validate(req.body);
    if (error) {
      throw createError(`Validation error: ${error.details[0]?.message}`, 400);
    }
  ...
  router.get('/api/version', asyncHandler(async (req: Request, res: Response) => {
    res.json({
      version: '0.5.1',
    });
  }));
  ...
  router.head('/api/blobs/:digest', asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post('/api/blobs/:digest', asyncHandler(async (req: Request, res: Response) => {
    const { digest } = req.params;
    
    // Handle blob upload (placeholder implementation)
    logger.info('Blob upload requested', { digest, contentLength: req.get('Content-Length') });
