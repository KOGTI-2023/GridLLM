...
export class RedisService {
  ...
  async publish(channel: string, message: string): Promise<void> {
    if (!this.publisherConnection) throw new Error('Redis publisher not connected');
    await this.publisherConnection.publish(channel, message);
  }

  async subscribe(channel: string, callback: (message: string) => void): Promise<void> {
    if (!this.subscriberConnection) throw new Error('Redis subscriber not connected');
  ...
  async psubscribe(pattern: string, callback: (channel: string, message: string) => void): Promise<void> {
    ...
    this.subscriberConnection.on('pmessage', (receivedPattern, channel, message) => {
