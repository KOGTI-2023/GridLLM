...
export class WorkerRegistry extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await this.redis.subscribe('worker:registered', this.handleWorkerRegistered.bind(this));
      await this.redis.subscribe('worker:unregistered', this.handleWorkerUnregistered.bind(this));
      await this.redis.subscribe('worker:heartbeat', this.handleWorkerHeartbeat.bind(this));
      await this.redis.subscribe('worker:status_update', this.handleWorkerStatusUpdate.bind(this));
    ...
    try {
      ...
      for (const [workerId, data] of Object.entries(workerData)) {
        try {
          ...
          const heartbeatAge = Date.now() - new Date(workerInfo.lastHeartbeat).getTime();
          ...
          if (heartbeatAge < config.server.workerHeartbeatTimeout) {
            ...
            logger.worker(workerId, 'Removed stale worker during initialization');
          }
        } catch (error) {
  ...
  private async handleWorkerRegistered(message: string): Promise<void> {
    try {
      ...
      // Store in Redis
      await this.redis.hset('workers', data.workerId, JSON.stringify(workerInfo));
  ...
  private async handleWorkerStatusUpdate(message: string): Promise<void> {
    try {
      ...
      if (worker) {
        ...
        logger.worker(data.workerId, 'Worker status updated', { status: data.status });
      }
    } catch (error) {
  ...
  getWorkersByModel(modelName: string): WorkerInfo[] {
    return this.getAllWorkers().filter(worker =>
      worker.capabilities.availableModels.some(model => model.name === modelName)
    );
  }
  ...
  async updateWorkerJobCount(workerId: string, increment: number): Promise<void> {
    const worker = this.workers.get(workerId);
    ...
    if (worker) {
      ...
      if (worker.currentJobs >= config.jobs.maxConcurrentJobsPerWorker) {
        worker.status = 'busy';
      } else if (worker.status === 'busy' && worker.currentJobs < config.jobs.maxConcurrentJobsPerWorker) {
        worker.status = 'online';
      }
  ...
  getTotalActiveJobs(): number {
    return this.getAllWorkers().reduce((total, worker) => total + worker.currentJobs, 0);
  }
