...
import { InferenceRequest, InferenceResponse, JobAssignment, WorkerInfo } from '@/types';
...
export class JobScheduler extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await this.redis.subscribe('job:completed', this.handleJobCompleted.bind(this));
      await this.redis.subscribe('job:failed', this.handleJobFailed.bind(this));
      await this.redis.subscribe('job:timeout', this.handleJobTimeout.bind(this));
  ...
  private async loadExistingJobs(): Promise<void> {
    try {
      ...
      logger.info(`Loaded ${this.activeJobs.size} active jobs and ${this.jobQueue.length} queued jobs`);
    } catch (error) {
  ...
  private async processJobQueue(): Promise<void> {
    ...
    this.jobQueue = this.jobQueue.filter(job => !processedJobs.includes(job.id));
  ...
  private selectWorkerForJob(job: InferenceRequest): WorkerInfo | null {
    ...
    const candidateWorkers = this.workerRegistry.getAvailableWorkersByModel(job.model);
    ...
    candidateWorkers.sort((a, b) => {
      ...
      return (tierOrder[b.capabilities.performanceTier] || 2) - (tierOrder[a.capabilities.performanceTier] || 2);
    });

    return candidateWorkers[0] || null;
  }

  private async assignJobToWorker(job: InferenceRequest, worker: WorkerInfo): Promise<void> {
    try {
      ...
      await this.redis.hset('active_jobs', job.id, JSON.stringify(jobAssignment));

      // Mark worker as busy
      await this.workerRegistry.markWorkerBusy(worker.workerId);

      // Send job to worker
      await this.redis.publish(`worker:${worker.workerId}:job`, JSON.stringify({
        type: 'job_assignment',
        job: jobAssignment,
      }));
  ...
  private async handleJobFailed(message: string): Promise<void> {
    try {
      ...
      if (jobAssignment) {
        ...
        const retryCount = (jobAssignment.request.metadata?.retryCount || 0) + 1;
  ...
  private async handleJobTimeout(message: string): Promise<void> {
    try {
      ...
      if (jobAssignment) {
        ...
        await this.redis.publish(`worker:${jobAssignment.workerId}:job`, JSON.stringify({
  ...
  private async waitForActiveJobs(): Promise<void> {
    ...
    if (this.activeJobs.size > 0) {
      logger.warn(`Forcibly stopping with ${this.activeJobs.size} active jobs remaining`);
    }
  }
  ...
  async submitAndWait(request: InferenceRequest): Promise<InferenceResponse> {
    return new Promise(async (resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`Job ${request.id} timed out after ${request.timeout}ms`));
  ...
  async cancelJob(jobId: string): Promise<boolean> {
    ...
    if (activeJob) {
      ...
      await this.redis.publish(`worker:${activeJob.workerId}:job`, JSON.stringify({
        type: 'job_cancellation',
        jobId,
      }));
